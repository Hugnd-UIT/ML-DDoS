...
def execute_extend(
    ...
    if ENFORCER_REF is not None:
        try:
            import time
            with ENFORCER_REF.lock:
                ENFORCER_REF.ban_registry[src_ip] = time.time() + ttl
                if ENFORCER_REF.blacklist_map is not None:
                    from prevent.enforcer import ip2int
                    key = ENFORCER_REF.blacklist_map.Key(ip2int(src_ip))
                    ENFORCER_REF.blacklist_map[key] = (
                        ENFORCER_REF.blacklist_map.Leaf(ttl)
                    )
            return json.dumps({
                "status": "ok",
                "action": "extend_ban",
...
def execute_command(
    ...
    try:
        ...
        return json.dumps({
            "status": "rejected",
            "message": f"Invalid command syntax: {err}"
        })

    if not tokens or tokens[0] not in allowed:
        return json.dumps({
            "status": "rejected",
            "message": "Command not in allowed network security tools list."
        })
